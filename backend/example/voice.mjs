...
function printUsage() {
  console.log("Usage: node voice.js [options]");
  console.log("");
  console.log("Options:");
  console.log("  --api-key <key>             VoiceLive API key");
  console.log("  --endpoint <url>            VoiceLive endpoint URL");
  console.log("  --api-version <version>     VoiceLive API version");
  console.log("  --model <name>              Model to use (default: gpt-realtime)");
  console.log("  --agent-name <name>         Foundry agent name for VoiceLive agent mode");
  console.log("  --agent-project-name <name> Foundry project name for VoiceLive agent mode");
  console.log("  --agent-version <version>   Foundry agent version for VoiceLive agent mode");
  console.log("  --model-mode                Force raw model mode instead of Foundry agent mode");
  console.log("  --voice <name>              Voice (default: en-US-Ava:DragonHDLatestNeural)");
...
function parseArguments(argv) {
  const parsed = {
    apiKey: readEnv("AZURE_VOICELIVE_API_KEY", "AZURE_VOICE_LIVE_API_KEY"),
    endpoint: readEnv("AZURE_VOICELIVE_ENDPOINT", "AZURE_VOICE_LIVE_ENDPOINT"),
    apiVersion: readEnv("AZURE_VOICELIVE_API_VERSION", "AZURE_VOICE_LIVE_API_VERSION"),
    model: readEnv("AZURE_VOICELIVE_MODEL", "AZURE_VOICE_LIVE_MODEL") ?? "gpt-realtime",
    agentName: readEnv("AZURE_VOICELIVE_AGENT_NAME", "AZURE_FOUNDRY_AGENT_NAME"),
    agentProjectName: readEnv("AZURE_VOICELIVE_PROJECT_NAME", "AZURE_VOICE_LIVE_PROJECT_NAME", "AZURE_FOUNDRY_PROJECT_NAME"),
    agentVersion: readEnv("AZURE_VOICELIVE_AGENT_VERSION", "AZURE_FOUNDRY_AGENT_VERSION"),
    forceModelMode: false,
    voice: readEnv("AZURE_VOICELIVE_VOICE", "AZURE_VOICE_LIVE_VOICE", "DEFAULT_VOICE_NAME") ?? "en-US-Ava:DragonHDLatestNeural",
    instructions:
      readEnv("AZURE_VOICELIVE_INSTRUCTIONS", "AZURE_VOICE_LIVE_INSTRUCTIONS") ??
      "You are a helpful AI assistant. Respond naturally and conversationally. Keep your responses concise but engaging.",
    audioInputDevice: readEnv("AUDIO_INPUT_DEVICE"),
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case "--agent-project-name":
        parsed.agentProjectName = argv[++i];
        break;
      case "--agent-version":
        parsed.agentVersion = argv[++i];
        break;
      case "--model-mode":
        parsed.forceModelMode = true;
        break;
      case "--voice":
        parsed.voice = argv[++i];
        break;
...
class BasicModelVoiceAssistant {
  constructor(options) {
    ...
    this.agentProjectName = options.agentProjectName;
    this.agentVersion = options.agentVersion;
    this.forceModelMode = options.forceModelMode;
  ...
  get useAgentMode() {
    return Boolean(!this.forceModelMode && this.agentName && this.agentProjectName);
  }
  ...
  _buildSessionTarget() {
    ...
    const agent = {
      ...
      projectName: this.agentProjectName,
    };

    if (this.agentVersion) {
      agent.agentVersion = this.agentVersion;
    }
  ...
  async start() {
    const clientOptions = this.apiVersion ? { apiVersion: this.apiVersion } : undefined;
    const client = new VoiceLiveClient(this.endpoint, this.credential, clientOptions);
    const sessionTarget = this._buildSessionTarget();
    const session = client.createSession(sessionTarget);
    this._session = session;

    if (this.useAgentMode) {
      console.log(
        `[init] Connecting to VoiceLive with Foundry agent "${this.agentName}" in project "${this.agentProjectName}" at "${this.endpoint}" ...`,
      );
    } else {
      console.log(`[init] Connecting to VoiceLive with model "${this.model}" at "${this.endpoint}" ...`);
    }

    this._subscription = session.subscribe({
      onConnected: async (_event, context) => {
        console.log(`[connection] Connected: ${context.endpoint}`);
      },

      onDisconnected: async (event, context) => {
        console.warn(`[connection] Disconnected from ${context.endpoint}: ${event.code} ${event.reason ?? ""}`.trim());
      },

      onError: async event => {
        console.error(`[connection] Error: ${event.error?.message ?? "Unknown connection error"}`);
      },

      onSessionCreated: async (event, context) => {
        console.log(`[session] Session created: ${context.sessionId}`);
        writeConversationLog(`SessionID: ${context.sessionId}`);
        if (event.session?.agent || this.useAgentMode) {
          writeConversationLog(`Agent Name: ${event.session?.agent?.name ?? this.agentName ?? ""}`);
          writeConversationLog(`Agent Project: ${this.agentProjectName ?? ""}`);
          writeConversationLog(`Agent Version: ${this.agentVersion ?? ""}`);
        }
      },
      ...
      onSessionUpdated: async (event, context) => {
        ...
        writeConversationLog(
          [
            ...
            `Agent Name: ${agent?.name ?? this.agentName ?? ""}`,
            `Agent Project: ${this.agentProjectName ?? ""}`,
    ...
    if (this.useAgentMode) {
      console.log("[session] Agent mode enabled: using Foundry-managed agent settings");
    } else {
  ...
  async _sendProactiveGreeting() {
    ...
    if (this.greetingText) {
      ...
      try {
        const openingText = this.useAgentMode
          ? "Please begin the live visa interview. Greet the applicant briefly as Officer Charles and ask the first interview question."
          : "Say something to welcome the user in English.";

        await session.addConversationItem({
          type: "message",
          role: this.useAgentMode ? "user" : "system",
          content: [
            {
              type: "input_text",
              text: openingText,
  ...
  async _setupSession() {
    ...
    if (this.useAgentMode) {
      // Agent instructions, tools, and temperature should stay in Foundry.
      // This update only configures realtime audio behavior.
    } else {
      sessionConfig.model = this.model;
      sessionConfig.instructions = this.instructions;
    }

    await this._session.updateSession(sessionConfig);
...
async function main() {
  ...
  if (!args.endpoint) {
    console.error("❌ Missing endpoint. Set AZURE_VOICELIVE_ENDPOINT or pass --endpoint.");
    process.exit(1);
  }

  if (!args.apiKey && !args.useTokenCredential) {
    console.error("❌ No authentication provided.");
    console.error("Provide --api-key / AZURE_VOICELIVE_API_KEY or use --use-token-credential.");
    process.exit(1);
  }

  const credential = args.useTokenCredential ? new DefaultAzureCredential() : new AzureKeyCredential(args.apiKey);
  const useAgentMode = Boolean(!args.forceModelMode && args.agentName && args.agentProjectName);

  console.log("Configuration:");
  console.log(`  AZURE_VOICELIVE_ENDPOINT: ${args.endpoint}`);
  console.log(`  AZURE_VOICELIVE_API_VERSION: ${args.apiVersion ?? "(SDK default)"}`);
  console.log(`  Session mode: ${useAgentMode ? "Foundry agent" : "Raw model"}`);
  console.log(`  AZURE_VOICELIVE_MODEL: ${args.model}`);
  if (useAgentMode) {
    console.log(`  AZURE_VOICELIVE_AGENT_NAME: ${args.agentName}`);
    console.log(`  AZURE_VOICELIVE_PROJECT_NAME: ${args.agentProjectName}`);
    console.log(`  AZURE_VOICELIVE_AGENT_VERSION: ${args.agentVersion ?? "(latest)"}`);
  }
  ...
  console.log(`  Authentication: ${args.useTokenCredential ? "DefaultAzureCredential" : "API Key"}`);
  ...
  const assistant = new BasicModelVoiceAssistant({
    ...
    agentProjectName: args.agentProjectName,
    agentVersion: args.agentVersion,
    forceModelMode: args.forceModelMode,
    voice: args.voice,
...
console.log("🎙️  Basic Voice Assistant with Azure VoiceLive SDK (Model/Agent Mode)");
console.log("=".repeat(60));
main().then(
